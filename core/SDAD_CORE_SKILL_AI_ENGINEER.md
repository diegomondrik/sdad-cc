...
- Does DECISIONS.md exist? If not, note it — $build will create it on first increment.

WHEN no test command found: flag immediately before Phase 1, not at Phase 3.
This is cheaper to fix before the first increment than after.
...
Flag gaps immediately. Tooling debt compounds faster than code debt.
...
### Phase 4 — QA
The Documentation layer in QA is owned by this skill. It runs as Layer 5 of 6:
  ...
  - Is there a /docs folder? If not, does this increment warrant creating it?
  - If Tier 2/3: is there an audit trail for the new user-affecting actions?
  - Is DECISIONS.md up to date with the architectural choices made in this increment?

DX check (runs alongside Documentation layer):
...
|----------|------|----------------|
| No test command in package.json / Makefile | QA is manual — scales to zero | Flag in Phase 0, add before first increment |
...
| DECISIONS.md missing after first $build | Lost architectural context | Create on first increment, never skip |
